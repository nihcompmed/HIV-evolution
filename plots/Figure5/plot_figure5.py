...
fig, axes = plt.subplots(N_rows, N_cols, 
                         figsize=(N_cols * 8, N_rows * 8),  # Adjusted size
...
for row_idx, drug in enumerate(do_drugs):
    for col_idx, treatment in enumerate(do_treatments):
        ...
        ax.legend(handles, labels, fontsize=36, loc='best')
        
        # --- Set Grid-Specific Labels ---
        
        # Set column titles (Treatment) on the top row
        if row_idx == 0:
            title_text = f'Treatment regimen\n{treatment_titles[col_idx]}'
            ax.set_title(title_text, fontsize=42, pad=15)
            
        # Set row labels (Drug) on the first column
        if col_idx == 0:
            ylabel_text = f'Resist {drug}'
            ax.set_ylabel(ylabel_text, fontsize=46)
            
        # Set X-label on the bottom row
        if row_idx == N_rows - 1:
            ax.set_xlabel(r'$\tau$', fontsize=46)
...
plt.savefig('fig5_LOO_grid.jpg', dpi=300, bbox_inches='tight')
plt.close()
